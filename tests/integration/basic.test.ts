...
describe('splice CLI integration', () => {
  ...
  const fixtureArchive = path.resolve(projectRoot, 'tests/integration/fixtures/archive');
  ...
  it('converts a minimal archive to Markdown threads and OAI JSONL, honoring system message', async () => {
    ...
    expect(fssync.existsSync(path.join(fixtureArchive, 'data', 'manifest.js'))).toBe(true);

    const systemMsg = 'Integration system message ✅';

    const { exitCode } = await execa(tsxBin, [
      cliEntry,
    ...
    const threadMdPath = path.join(threadsDir, threadFiles.find(f => f.endsWith('.md')) as string);
    ...
    const lines = oaiRaw.split('\n').map(l => l.trim()).filter(Boolean);
    expect(lines.length).toBeGreaterThan(0);

    const firstRecord = JSON.parse(lines[0]);
    expect(Array.isArray(firstRecord.messages)).toBe(true);
    expect(firstRecord.messages[0]).toEqual({ role: 'system', content: systemMsg });
